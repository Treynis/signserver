 ...
 *   read-only = "true"
 *   
 *   @version $Id: RSASignSessionBean.java 8422 2009-12-10 13:18:39Z primelars $
 */
        ...
        try {
            ...
            for (int i = 0; i < certs.length; i++) {
                ...
                if (ci == null) {
                    ...
                    certificateStore.storeCertificate(admin, cert, name, cafp, CertificateDataBean.CERT_ACTIVE, type, profileId, tag, updateTime);
                } else {
    ...
    private Certificate createCertificate(Admin admin, UserDataVO data, X509Name requestX509Name, CA ca, PublicKey pk, int keyusage, Date notBefore, Date notAfter, X509Extensions extensions, String sequence) throws IllegalKeyException {
        ...
        try {
            ...
            if (data.getType() == SecConst.USER_INVALID) {
                ...
                if (publishers != null) {
                    pub.storeCertificate(admin, publishers, cert, data.getUsername(), data.getPassword(), data.getDN(), cafingerprint, CertificateDataBean.CERT_ACTIVE, certProfile.getType(), -1, RevokedCertInfo.NOT_REVOKED, tag, certProfileId, updateTime, data.getExtendedinformation());
                }
